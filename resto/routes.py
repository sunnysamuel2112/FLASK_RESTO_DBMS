...
def menucust():
    ...
    if add_dish.validate_on_submit():
        print(add_dish)
    menutodisplay = menu.query.all()
    orderitems = db.session.query(order, menu).join(menu).all()
    total = 0
    for o, m in orderitems:
        total = total + (m.dish_price * o.ord_dish_count)

    return render_template('menuforcust.html', total=total, menutodisplay=menutodisplay, orderitems=orderitems, add_dish=add_dish)
...
def cartadd(dish_id):
    ...
    last = customer.query.order_by(customer.cust_id).first()
...
def adminupteam(emp_id):
    form = updateEmpForm()
    emp_to_update = menu.query.get_or_404(emp_id)

    if request.method == 'POST':
        emp_to_update.emp_name = form.empname.data,
        emp_to_update.emp_desg = form.empdesg.data,
        emp_to_update.emp_phno = form.empphno.data,
        emp_to_update.emp_salary = form.empsalary.data,
        emp_to_update.emp_address = form.empaddress.data
        try:
            db.session.commit()
            flash('Employee Details Updated', category='success')
            return redirect('adminteam')
        except:
            flash('Error Updating Employee Details!', category='danger')
            return redirect('adminteam')

    return render_template('upemp.html',  emp_to_update=emp_to_update, form=form)
